...
function startGame() {
    ...
    console.log('Random Number:', randomNumber);  //for checking in console
...
function createGrid() {
    ...
    gridContainer.innerHTML = '';  // Clear the grid if restarting
...
function handleGuess(number, ball) {
    if (attemptsLeft > 0) {
        ...
        if (attemptsLeft === 0 && number !== randomNumber) {
            setMessage(`You have no more attempts left. You lose! Actual Number was ${randomNumber}`);
            disableAllButtons();
            document.getElementById('restart-btn').style.display = 'block';
        }
    }
